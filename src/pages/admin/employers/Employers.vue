...
<template>
    <PageContainer :breadcrumbs="breadcrumbs">
        ...
        >
            <template #columns>
                <Column field="id" header="ID" dataType="numeric" headerStyle="width: 3rem; min-width: 3rem;" sortable>
                    <template #filter="{ filterModel }">
                        <InputText v-model="filterModel.value" type="number" style="width: 10rem;"
                ...
                <Column field="position" header="Должность" sortable>
                    <template #filter="{ filterModel }">
                        <Dropdown v-model="filterModel.value" :options="positions" placeholder="Все"
            ...
            <template #buttons>
                <Button v-if="user.access.employers === 2" type="button" icon="pi pi-plus" label="Добавить" outlined
