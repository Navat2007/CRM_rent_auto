...
import DirectoryOperationTypesEditForm
    from "@components/Forms/Directory/OperationTypes/DirectoryOperationTypesEditForm.vue";
...
const directoryParentTitle = 'Тип операции';
const directoryTitle = 'Редактирование типа операции';
...
const handleEdit = (data) => {
  ...
  DirectoryService.editOperationType(data).then((response) => {
    if (response.data) {
...
<template>
  <PageContainer :loading="loading" :breadcrumbs="breadcrumbs">
    <DirectoryOperationTypesEditForm :title="directoryTitle" @onSubmit="handleEdit" @onDelete="isDeleteModalOpen = true" :item="item"/>
