...
async function fetchData() {
  items.value = await DirectoryService.getPositions(user.company_id);
  loading.value = false;
}
...
<template>
  <PageContainer :breadcrumbs="breadcrumbs">
    <Table
        title="Должности" :items="items" :columns="columns"
    ...
    >
      <template #columns>
        ...
        <Column field="archive" header="Статус" sortable>
          <template #filter="{ filterModel }">
            <Dropdown v-model="filterModel.value" :options="statuses" placeholder="Все" class="p-column-filter" showClear/>
          </template>
        </Column>
      </template>
      <template #buttons>
