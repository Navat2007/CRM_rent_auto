  ...
  <Card v-if="variant === 'primary'" style="overflow: hidden" pt:root="sm:flex-row" pt:header="sm:flex-auto sm:w-1/2" pt:body="sm:flex-auto sm:w-1/2">
    ...
    <template #title>
      <div class="flex justify-between">
        ...
        <span class="flex items-center text-sm text-gray-800 border-2 border-dashed rounded-xl p-1">{{item.state_number}}</span>
      </div>
    ...
    <template #content>
      <div class="grid gap-2 grid-cols-1 mt-2">
        ...
        <div class="flex justify-between"><span>Комплектация</span><span class="font-bold">{{item.configuration}}</span></div>
        <div class="flex justify-between"><span>Поколение</span><span class="font-bold">{{item.generation}}</span></div>
        <div class="flex justify-between"><span>Тип топлива</span><span class="font-bold">{{item.fuel_type}}</span></div>
      </div>
    </template>
    <template #footer>
