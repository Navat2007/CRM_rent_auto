        ...
        <template #content>
            <Tabs :value="tabValue" scrollable>
                <TabList>
                    ...
                    <Tab value="1" class="flex gap-2">Прочие документы</Tab>
                    <Tab value="2" class="flex gap-2">Права доступа</Tab>
                    <Tab value="3" class="flex gap-2">Проверки в odyssey</Tab>
                    <Tab value="4" class="flex gap-2">Проверки в himera</Tab>
                    ...
                    <TabPanels>
                        ...
                        { title: 'Кем приходится', value: 'who' }
                                ...
                                <div>
                                    ...
                                    <Select v-model="state.position" :loading="loadingPositions" :options="positions"
                                            optionLabel="name"
                                            optionValue="id" placeholder="Выберите должность" showClear filter
                                            class="w-full">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <div class="flex gap-2">
                                        <InputMask id="inn" v-model="state.inn" :modelValue="state.inn"
                                                   mask="999999999999"
                                                   placeholder="999999999999"/>
                                        <Button pt:root="min-w-32" type="button" icon="pi pi-upload" label="Запросить"
                                                :loading="uploadINN" outlined
                                                @click="() => {
                              uploadINN = true;
                              apiCloudRef.sendNalogINNRequest();
                                ...
                                <div>
                                    <label for="actsOnBasis"
                                           class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Действует
                                        на
                                        основании</label>
                                    <input
                                ...
                                <div>
                                    <label for="firingDate"
                                           class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Дата
                                        увольнения</label>
                                    <DatePickerWithMask :value="state.firingDate"
                                                        @onChange="e => state.firingDate = e"/>
                                </div>
                                <div v-if="state.archive === 0" class="grid gap-4 my-4 sm:grid-cols-1">
                                    ...
                                    <div>
                                        <label for="password"
                                               class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Новый
                                            пароль</label>
                                        <Password inputId="password" v-model="state.password"
                                    ...
                                    <div>
                                        <label for="confirmPassword"
                                            ...
                                            пароль</label>
                                        <Password inputId="confirmPassword" v-model="state.confirmPassword"
                                                  placeholder="Введите пароль"
                                                  :toggleMask="true"
                                                  :feedback="false" class="w-full mb-3" inputClass="w-full"
                                                  :disabled="passwordDisabled"/>
                        ...
                        <TabPanel value="2">
                            <Fieldset legend="Управление справочниками">
                                <div v-for="item in access" :key="'access_directory' + item.key"
                                     class="flex items-center">
                                    <RadioButton v-model="state.access_directory"
                                                 :inputId="'access_directory' + item.key" name="dynamic"
                                                 :value="item.key"/>
                                    <label :for="item.key" class="ml-2">{{ item.name }}</label>
                                </div>
                            </Fieldset>
                            <Fieldset legend="Управление сотрудниками">
                                <div v-for="item in access" :key="'access_employers' + item.key"
                                     class="flex items-center">
                                    <RadioButton v-model="state.access_employers"
                                                 :inputId="'access_employers' + item.key" name="dynamic"
                                                 :value="item.key"/>
                                    <label :for="item.key" class="ml-2">{{ item.name }}</label>
                                </div>
                            </Fieldset>
                            <Fieldset legend="Управление клиентами">
                                <div v-for="item in access" :key="'access_clients' + item.key"
                                     class="flex items-center">
                                    <RadioButton v-model="state.access_clients" :inputId="'access_clients' + item.key"
                                                 name="dynamic"
                                                 :value="item.key"/>
                                    <label :for="item.key" class="ml-2">{{ item.name }}</label>
                                </div>
                            ...
                            <Fieldset legend="Управление автомобилями">
                                <div v-for="item in access" :key="'access_auto' + item.key" class="flex items-center">
                                    <RadioButton v-model="state.access_auto" :inputId="'access_auto' + item.key"
                                                 name="dynamic"
                                                 :value="item.key"/>
                                    <label :for="item.key" class="ml-2">{{ item.name }}</label>
                                </div>
                            ...
                            <Fieldset legend="Управление бронированием">
                                <div v-for="item in access" :key="'access_booking' + item.key"
                                     class="flex items-center">
                                    <RadioButton v-model="state.access_booking" :inputId="'access_booking' + item.key"
                                                 name="dynamic"
                                                 :value="item.key"/>
                                    <label :for="item.key" class="ml-2">{{ item.name }}</label>
                                </div>
                        ...
                        <TabPanel value="3">
                            ...
                            >
                                <template #columns>
                                    <Column field="created_at" dataType="date" header="Дата"
                                    ...
                                    <Column field="scoring_overall_indicator" header="Cкоринг" dataType="numeric"
                                        ...
                                        <template #filter="{ filterModel }">
                                            <InputText v-model="filterModel.value" type="number"
                                                       placeholder="Поиск по скорингу"/>
                                        </template>
                                    </Column>
                                    ...
                                    <Column field="firstname" header="Имя" headerStyle="width: 10rem; min-width: 10rem;"
                                            sortable>
                                        <template #filter="{ filterModel }">
                                            <InputText v-model="filterModel.value" type="text"
                                    ...
                                    <Column header="Полный отчет" headerStyle="width: 8rem; min-width: 8rem;">
                                        <template #body="slotProps">
                                            <Button type="button" @click="openOdysseyUrl(slotProps.data.url)"
                                                    icon="pi pi-search" severity="secondary" rounded></Button>
                                        </template>
                                    </Column>
                        ...
                        <TabPanel value="4">
                            ...
                            >
                                <template #columns>
                                    ...
                                    <Column header="Полный отчет" headerStyle="width: 8rem; min-width: 8rem;">
                                        <template #body="slotProps">
                                            <Button type="button" @click="openHimeraUrl(slotProps.data.url)"
                                                    icon="pi pi-search"
